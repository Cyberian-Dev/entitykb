...
def test_add_user(user_store: UserStore):
    password = user_store.add_user(
        username="ian", status=UserStatus.read_write
    )
    assert len(password) > 15 and 3 == password.count("-")

    user: StoredUser = user_store.get_user("ian")
...
def test_add_user_auth(user_store: UserStore):
    pw = user_store.add_user(username="ian", status=UserStatus.read_write)
    token = user_store.authenticate(username="ian", password=pw)
    status = user_store.get_user_status(token=token)
    assert status == UserStatus.read_write


def test_set_status(user_store: UserStore):
    user_store.add_user(username="ian", status=UserStatus.read_only)
    ...
    user_store.set_status(username="ian", status=UserStatus.read_write)
...
def test_reset_password(user_store: UserStore):
    pass_1 = user_store.add_user(username="ian", status=UserStatus.read_write)

    pass_2 = user_store.reset_password(username="ian")
...
def test_invalid_user(user_store: UserStore):
    ...
    user_store.add_user(username="ian", status=UserStatus.read_write)
